...
# plotting functions:
plot_ais_0.2_1 <- function(fc, ylim = c(0, max(fc$ais)), ylab = expression(AIS[list(0, 0.2)]), ...){
...
# #------------------------------------
# # Plot for AIS with alpha = c(0, 0.2)
...
# axis(2); box()
# lines(c(0.5*mean(scores_kcde_0.2_1$width_pi.alpha.0.2 + scores_kcde_0.2_1$penalty.alpha.1),
#         0.5*mean(scores_sarima_0.2_1$width_pi.alpha.0.2 + scores_sarima_0.2_1$penalty.alpha.1)),
#       col = "lightcoral", lwd = 2, type = "h")
# lines(c(0.5*mean(scores_kcde_0.2_1$width_pi.alpha.0.2),
#         0.5*mean(scores_sarima_0.2_1$width_pi.alpha.0.2)),
...
plot_ais(scores_kcde_0.2, ylim = c(0, 3), ylab = expression(IS[0.2]))
legend("top", legend = c("penalty for non-coverage of 80% PI", "width of 80% PI"),
...
legend("topleft", legend = c("median", "80% PI", "observed"),
       lty = c(2, 1, NA), pch = c(NA, NA, 15), bty = "n",
